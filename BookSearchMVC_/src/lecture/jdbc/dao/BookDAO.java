...
public class BookDAO {
	...
	static {
		...
		basicDS.setUrl(
				"jdbc:mysql://127.0.0.1:3306/mysql_test_db?characterEncoding=UTF-8&serverTimezone=UTC&useSSL=false&allowPublicKeyRetrieval=true");
	...
	public ObservableList<BookVO> select(String text) {
		...
		try {
			...
			while (rs.next()) {
				BookVO bookvo = new BookVO(rs.getString("bisbn"), rs.getString("btitle"), rs.getString("bauthor"),
						rs.getInt("bprice"));
				list.add(bookvo);
			}
			rs.close();
	...
	public ObservableList<BookVO> detail(String detailISBN) {
		DataSource ds = getDataSource();
		Connection con = null;
		ObservableList<BookVO> list = null;
